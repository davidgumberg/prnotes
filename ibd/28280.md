...
## [coins: pass linked list of flagged entries to BatchWrite](https://github.com/bitcoin/bitcoin/pull/28280/commits/7825b8b9aeceb4ff607650cdc9c49e5de9c7719f)
...
The cursor is the mechanism we will use to traverse the doubly linked list.

This commit modifies `BatchWrite` to use `CoinsViewCacheCursor` to iterate over
the linked list of flagged entries instead of iterating over the entire coins
cache.

It has methods `Begin` which returns the Sentinel's next (the oldest entry):
```cpp
    inline CoinsCachePair* Begin() const noexcept { return m_sentinel.second.Next(); }
```

and `End` which returns the sentinel:

```cpp
    inline CoinsCachePair* End() const noexcept { return &m_sentinel; }
```

Most importantly, it has a method `NextAndMaybeErase` which as of this commit,
only gets the next entry in the linked list given a `CoinsCachePair& current`:
...
`CoinsViewCacheCursor` annotated
...
{
    // [ These attributes are marked LIFETIMEBOUND to indicate that
    //   CoinsViewCacheCursor should never outlive them ]
    ...
    // [ The oldest entry is the Next() of the sentinel. ]
    inline CoinsCachePair* Begin() const noexcept { return m_sentinel.second.Next(); }

    // [ The youngest entry is the Prev() of the sentinel and the sentinel is
    //   the 'end'. ]
    inline CoinsCachePair* End() const noexcept { return &m_sentinel; }

    // [ This will be more fleshed out in a subsequent commit. ]
    inline CoinsCachePair* NextAndMaybeErase(CoinsCachePair& current) noexcept { return current.second.Next(); }
...
### Changes to BatchWrite

The bulk of the changes in this commit are to BatchWrite:
...
This bears one of the major performance improvements of this PR, iterating over
*only* the flagged coins on a flush/sync instead of every single coin in the
cache, this will pair well with a later commit that lets us keep a bigger cache
around for longer.

Another change is that whether or not we are erasing is now tracked by the
cursor, which will become responsible for erasing entries in a later commit, but
BatchWrite, which only cares about whether or not we are erasing for performance
reasons and not because it's erasing, now does:
...
### Changes to Flush and Sync

Flush and Sync changes are mostly due to the changes in interface, we construct
a cursor and pass a ref to that, instead of passing a reference to the map of
cached coins.
...
BatchWrite is no longer responsible for clearing the coins cache at the end of
the flush:
...
Flush now takes care of clearing our cache before the `ReallocateCache()` call.

## [coins: move Sync logic to CoinsViewCacheCursor](https://github.com/bitcoin/bitcoin/pull/28280/commits/05cf4e18758618bb493d26014d3a9c89bf28d898)

Previously, after CCoinsViewCache::Sync()'s call to BatchWrite completed, `Sync`
would loop through every coin in our cache and clear the flags of any unspent
coins, since after a flush DIRTY coins would no longer be DIRTY, and any unspent
coins that were FRESH (our parent didn't know about them) are now known to the
parent view. And, delete any spent coins from the cache, since we no longer need
them. A lot of this is described in [this comment](https://github.com/bitcoin/bitcoin/pull/28280/commits/05cf4e18758618bb493d26014d3a9c89bf28d898#discussion_r1676224407)
by the PR author.
...
bool CCoinsViewCache::Sync()
{
    ...
    // [ Iterate through each coin in the cache... ]
    for (auto it = cacheCoins.begin(); it != cacheCoins.end(); ) {
        if (it->second.coin.IsSpent()) {
            ...
            // [ Clear the flags of the unspent, since if they were DIRTY,
            //   they've been flushed and aren't DIRTY anymore, and if they were
            //   FRESH, they are no longer since our parent has them now. ]
...
Now, sync just does a sanity check that our `m_sentinel` is connected to itself,
which would be the case after all flagged entries have their flags cleared and
are removed from the linked list, and the old logic here gets moved to the
`CoinsViewCacheCursor` below..
...
Here is the logic now placed inside of
`CoinsViewCacheCursor::NextAndMaybeErase`, which is called by the loop inside of
`CCoinsViewCache::BatchWrite()` (see more details above):
...
inline CoinsCachePair* NextAndMaybeErase(CoinsCachePair& current) noexcept
{
    ...
    // [ m_will_erase is true if we're flushing, false if we're syncing, so
    //   we're syncing below... ]
    if (!m_will_erase) {
        if (current.second.coin.IsSpent()) {
            // [ identical to the old behavior, we erase any spent coins ]
            m_usage -= current.second.coin.DynamicMemoryUsage();
            m_map.erase(current.first);
        } else {
            // [ and clear the flags of any unspent coins. ]
...
This reduces the number of entries that need to be iterated through to only
those that were flagged at the start of the `BatchWrite`.

This commit also changes `CoinViewCacheCursor::WillErase()`:
...
This only affects the loop in `BatchWrite`, which as an optimization std::moves
entries that are going to be deleted instead of copying them:
...
## [Add linked-list test to CCoinsViewCache::SanityCheck](https://github.com/bitcoin/bitcoin/pull/28280/commits/0e8918755f725b6269ed2be5a0b46f1611233515)

`CCoinsViewCache::SanityCheck()` is used by the fuzzer as an additional set of
checks that nothing insane has happened, this commit adds a set of checks to it
to ensure that the size of the linked list is equal to to the number of flagged
entires in the coins cache, and to ensure that any item's next has a prev
pointing to itself, and any item's prev has a next pointing to itself, and that
the coins are flagged
...
## [validation: don't erase coins cache on prune flushes](https://github.com/bitcoin/bitcoin/pull/28280/commits/589db872e116779ab9cae693171ac8a8c02d9923)

### `FlushStateToDisk()`

This commit modifies `Chainstate::FlushStateToDisk()`, which takes a parameter
`FlushState
...
- The use of sync/flush should be reevaluated because of the changes to flushing
  behavior, namely

<details>

<summary>Outdated comment</summary>
...
- I'm nervous about the prospect of a dangling reference if an entry has it's
  ...
  this doesn't seem possible today, but is a future footgun for people touching
  this code.
    - This has been addressed in a later rebase which adds an Assume(m_flags) to
      `Next` and `Prev` to ensure that we are never traversing the linked list
      where an entry has been destructed. This is a great example of the utility
      of `Assume`, which is an `Assert` in debug builds and a no-op in release
      builds, to guard against at least the most trivial class of unintended
      future changes.
