...
- `CWallet::Create()` enforces as a matter of internal logic (with assert) that
  `WALLET_FLAG_DESCRIPTORS` will never be one of the `create_flags`, but
  `CreateWallet()` throws an error even though the wallet creation rpc disallows
  descriptors, are we less confident that `CreateWallet()` can be invoked with
  descriptors == false?
  (w0xlt)
...
// [ I hadn't seen `WalletInit` before, this is the only implementation of the
//   WalletInitInterface 'interface' class, introduced in
//   [#10762](https://github.com/bitcoin/bitcoin/pull/10762), this basically pulled
//   initialization logic that was related to the wallet out of `init.cpp`, and put
//   it in a class with static functions, maybe this pattern should be
//   revisited, since it seems a bit overkill, could those initialization functions
//   not go in CWallet? ]
...
@@ -5,8 +5,6 @@
 ...
 // [ A file generate during build, necessary for the build config ifdef's,
 //   can be remove now that the last build config check in this file is gone. ]
-#include <bitcoin-build-config.h> // IWYU pragma: keep
-
 #include <addresstype.h>
 #include <wallet/db.h>
...
+            addr_info = wallet_prev.getaddressinfo(address)
+
            #  [ using the backupwallet rpc guarantees everything is in one
            # file. some older versions would write to a variety of files, and
            # otherwise we'd have to do something complicated to move wallets
            # between nodes here. ]
