...
Now, to summarize in my own reading what another reviewer [has described](https://github.com/bitcoin/bitcoin/pull/30866#discussion_r1831410534) as where this problem is being triggered, in `src/wallet/test/fuzz/scriptpubkeyman.cpp` there is a utility function: , it is specifically in this utility function with the following signature:
...
The solution most in line with the style used for making copies of descriptors
elsewhere should they ever be the non multipath subexpression in a multipath
world is to introduce a `Clone()` function to `miniscript::Node` that will be
used in `MiniscriptDescriptor::Clone()`:
...
This `Clone()` function will have to be a little bit better than naive to avoid
the same recursion stack overflow issue as the destructor, here we take
advantage of some built in utilities for applying a recursive algorithm to a
miniscript tree in a manner that isn't actually recursive
`miniscript::Node::TreeEval*()`. The comments above the TreeEval functions
[suggest](https://github.com/bitcoin/bitcoin/blob/477b357460743a9304cccfb672e5445f364891ae/src/script/miniscript.h#L561-L568)
...
applied from leaf to tip (up) takes two parameters: a node that is currently
...
struct Node {
    ...
    NodeRef<Key> Clone() const
    {
        ...
        auto upfn = [](const Node& node, Span<NodeRef<Key>> children) {
            // [ This makes a shallow copy of the node in the same manner as
            //   above. ]
            NodeRef<Key> ret(MakeNodeRef<Key>(node));
            // [ Delete all the sub pointers to avoid the dangling. ]
            ret->subs.clear();
            // [ Push all the NodeRef's of the children into this node's subs. ]
            for (const auto& child : children) {
                ret->subs.push_back(child);
            }
            // [ Return the result of our operation, a NodeRef to this node.
            return ret;
        };
