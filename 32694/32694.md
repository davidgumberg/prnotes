...
void BaseIndex::Sync()
{
    ...
    if (!m_synced) {
        ...
        while (true) {
            ...
            if (!pindex_next) {
                // [ This is the important part of if consequences: set the best
                //   block index and commit to disk, the next part is just about
                //   setting `m_synced = true`. To be safe when doing this, we need
                //   to take a lock on main and make sure that the blocks haven't
                //   advanced. We do it this way because we don't want to lock
                //   `cs_main` on an index flush which might take a while. ]
                SetBestBlockIndex(pindex);
            ...
            // [ This is related to the details of `NextSyncBlock` from above,
            //   it returns the next block from `pindex` *if* it has a next block
            //   in this chain, if it doesn't, it might be because pindex is not
            //   in the block, or it might be because it's the tip (LOL).
            //   If it doesn't find a next block of pindex then it returns the
            //   next block from the common block of pindex's chain and the global
            //   chain, if it was the tip, this will be nullptr again. If pindex
            //   is in fact not on the global chainstate, then we need to rewind
            //   the index, seems a bit dangerous to me that Rewind's can be
            //   blocking, is it possible to stall a node with a cleverly
            //   designed re-org? ]
            ...
            if (!m_chainstate->m_blockman.ReadBlock(block, *pindex)) {
                ...
                // [ I think this is so ugly, the `if` branch returns, no need
                //   for else! ]
                block_info.data = &block;
            }
            // [ "Custom*" is the naming convention used for the functions
            //   overriden by the derived classes, this is where the index will
            //   take the block and do it's thing with it.. ]
...
{
    // [ We have to be starting from the right place! ]
    assert(current_tip == m_best_block_index);
    // [ We better be rewinding to somethning that's on current_tip's chain! ]
    assert(current_tip->GetAncestor(new_tip->nHeight) == new_tip);

    // [ Hand-off to the index to do it's thing with the rewind. CustomRewind
    //   takes two `interfaces::BlockRef&` that is a hash and a height. ]
...
Let's look at a specific `CustomRewind()` instance:
...
#### `CoinStatsIndex::CustomAppend` annotated
...
This commit is pretty simple, and could probably even be a ranged-diff, in the
index unit tests there is this common pattern of starting background syncing
...
This can be replaced with just a blocking sync:
```cpp
    filter_index.Sync();
```

And that's what this commit does across all of the index unit tests.
...
This changes `IsBip30Unspendable()` in a really minimal way, instead of taking a
`CBlockIndex` and getting hash and height from there, it takes a hash and height
directly and checks those.
